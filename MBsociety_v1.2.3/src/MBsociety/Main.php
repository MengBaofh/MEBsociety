...
class Main extends PluginBase implements Listener
{
	...
	public function onEnable()
	{
		...
		$this->sm = new Config($this->getDataFolder() . "Explain123.txt", Config::YAML, array(
			"##Q:商店怎么设置" => "A:复制粘贴模板",
			"##Q:每个商店配置文件的最前面的数字是什么意思?" => "A:这是商店序号，必须，必须，必须按数字顺序填写，否则可能出现未知bug!!!(仅支持1-2000)",
			"##配置文件改动情况" => "无",
			"请删除旧explain122.txt(如果存在)"
		));
		...
		if ($this->config->get("版本(勿动)") !== $this->getDescription()->getVersion() or $this->config->get("版本(勿动)") == null) {
			$this->config->set("版本(勿动)", "1.2.3");
			$this->config->save();
			//rename($this->getDataFolder() . "信息开关.yml", $this->getDataFolder() . "信息开关_old.yml");
			$this->getLogger()->info("§c检测到你的配置文件不是最新版本，已为你自动更新配置文件,详见Explain123.txt。");
			$this->getLogger()->info("§c--------------------------------");
		} else {
