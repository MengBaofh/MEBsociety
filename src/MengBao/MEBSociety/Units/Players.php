...
class Players
{
    ...
    public function setVip(string $playerName, int $day, bool $type = true): void
    {
        ...
        */
        ...
        if ($vips[$playerName]["day"] === 0) {
            unset($vips[$playerName]);  //0表示取消权限，直接把这条记录删掉
        }
        $config->setAll($vips);
        $config->save();
    }
    ...
    public function setAllVipDay(int $day, bool $type = true): void
    {
        $config = $type === true ? $this->plugin->vips : $this->plugin->svips;
        $vips = $config->getAll();
        //要按$type取名单，不然刷新svip的时候会去翻vip的名单
        foreach ($this->getVips($type) as $playerName) {
            if ($this->getPresiceVipDay($playerName, $type) === -1)
                continue;  //永久的不动
            $vips[$playerName]["day"] += $day;
            if ($vips[$playerName]["day"] <= 0)
                unset($vips[$playerName]);  //到期了就删掉，否则day为0还会被当成vip
        }
        $config->setAll($vips);
        $config->save();
    }
    ...
    public function addPrefix(string $playerName, string $prefix): void
    {
        ...
        if (!isset($prefixes[$playerName]))
            $prefixes[$playerName] = array();  //玩家称号表还没建好时先补一个，不然array_push会报错
    ...
    {
        //Config::get查不到时返回false，直接交出去会让in_array/foreach报类型错误
        $prefixes = $this->plugin->prefixes->get($playerName);
